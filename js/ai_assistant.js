...
- 總覽：KPI 卡片與整體資料概覽圖表。
- 地圖：Leaflet 地圖，可縣市/鄉鎮跳轉、座標定位即時環域統計、熱點查詢（1000易肇事路口／799人行安全補助點位／科技執法設備地點，查點位半徑內有多少「事故」）、易肇事路口科技執法涵蓋查詢（查1000易肇事路口或799人行安全補助點位半徑內有多少「科技執法設備」＋最近一處距離，方向與熱點查詢相反，可選單一路口或合計目前篩選出的所有路口去重查詢，並可匯出CSV，計算邏輯與報告圖3-2緩衝區比對表相同），A1死亡逐點或熱力圖、A2受傷熱區密度、各類點位圖層可自由勾選組合顯示。
...
- 覆蓋率分析：計算官方認定的高風險點位（如1000易肇事路口）中，有多少比例在特定距離內設有科技執法設備，並計算至最近一處設備的直線距離；直線距離採等距圓柱投影近似公式（非大圓距離，短距離下誤差可忽略）。報告內的「圖3-2」為固定半徑（100/300/500公尺）之靜態彙整表；若使用者想查任意路口在任意自訂半徑內的即時涵蓋數字，可到「地圖」分頁的「易肇事路口科技執法涵蓋查詢」卡片查詢，不需要再另外產生報表。
